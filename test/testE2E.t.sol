...
contract E2ETests is Test {
    ...
    function testExecuteVotesOnMultipleParachains() public {
        ...
        uint256 _aliceStartBalance = token.balanceOf(address(alice));
        vm.startPrank(alice);
        ...
        uint256 _darylStartBalance = token.balanceOf(address(daryl));
        vm.startPrank(daryl);
        ...
        // Check balances
        (, uint256 _bobStakedBal, uint256 _bobLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId, bob);
        assertEq(1, _bobStakedBal + _bobLockedBal);
        (, uint256 _aliceStakedBal, uint256 _aliceLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId2, alice);
        assertEq(2, _aliceStakedBal + _aliceLockedBal);
        (, uint256 _darylStakedBal, uint256 _darylLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId3, daryl);
        assertEq(3, _darylStakedBal + _darylLockedBal);
        assertEq(token.balanceOf(address(bob)), _bobStartBalance - 1);
        assertEq(token.balanceOf(address(alice)), _aliceStartBalance - 2);
        assertEq(token.balanceOf(address(daryl)), _darylStartBalance - 3);
        ...
        // Check balances
        (, _bobStakedBal, _bobLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId, bob);
        assertEq(0, _bobStakedBal + _bobLockedBal); // 1 - 3 = 0 (bob's stake was slashed)
        (, _aliceStakedBal, _aliceLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId2, alice);
        (, _aliceStakedBal, _aliceLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId2, alice);
        assertEq(0, _aliceStakedBal + _aliceLockedBal); // 2 - 2 = 0 (alice's stake was slashed)
        (, _darylStakedBal, _darylLockedBal,,,,,,) = staking.getParachainStakerInfo(fakeParaId3, daryl);
        assertEq(2, _darylStakedBal + _darylLockedBal); // 3 - 1 = 2 (daryl's stake was slashed)
        assertEq(token.balanceOf(address(bob)), _bobStartBalance - 1); // bob's token holdings should not have changed
        assertEq(token.balanceOf(address(alice)), _aliceStartBalance - 2); // alice's token holdings should not have changed
        assertEq(token.balanceOf(address(daryl)), _darylStartBalance - 3); // daryl's token holdings should not have changed
